...
static void n_wifi_socket_cb(SOCKET sock, uint8_t message_type,
                             void *message_data);
...
uint8_t WifiMgr::connect(const char *ssid, const char *psk) {
  ...
  int ret = m2m_wifi_connect((char *)ssid, strlen(ssid), M2M_WIFI_SEC_WPA_PSK,
                             (void *)psk, M2M_WIFI_CH_ALL);
...
void WifiMgr::handle_event(uint8_t message_type, void *message_data) {
  ...
  case M2M_WIFI_RESP_CON_STATE_CHANGED:
    ...
    handle_resp_conn_state_changed(
        static_cast<tstrM2mWifiStateChanged *>(message_data));
    break;
...
void WifiMgr::handle_resp_conn_info(tstrM2MConnInfo *info) {
  n_log("Connection state:\nIP Address: %u.%u.%u.%u\nMAC: "
        ...
        info->au8IPAddr[0], info->au8IPAddr[1], info->au8IPAddr[2],
        info->au8IPAddr[3], info->au8MACAddress[0], info->au8MACAddress[1],
        info->au8MACAddress[2], info->au8MACAddress[3], info->au8MACAddress[4],
        info->au8MACAddress[5], info->s8RSSI);
}
...
void WifiMgr::handle_resp_conn_state_changed(
    tstrM2mWifiStateChanged *new_state) {
  n_log("Wifi connection state changed to %s",
        new_state->u8CurrState == M2M_WIFI_CONNECTED ? "connected"
                                                     : "disconnected");
...
bool WifiMgr::has_ip_address() { return _has_ip_addr; }

tenuM2mConnState WifiMgr::connection_state() { return _connection_state; }

void WifiMgr::handle_socket_event(SOCKET sock, uint8_t message_type,
                                  void *message_data) {
...
bool is_m2m_config_cmd(uint8_t cmd) {
  return cmd >= M2M_CONFIG_CMD_BASE && cmd < M2M_WIFI_MAX_CONFIG_ALL;
}
bool is_m2m_sta_cmd(uint8_t cmd) {
  return cmd >= M2M_STA_CMD_BASE && cmd < M2M_WIFI_MAX_STA_ALL;
}
bool is_m2m_ap_cmd(uint8_t cmd) {
  return cmd >= M2M_AP_CMD_BASE && cmd < M2M_WIFI_MAX_AP_ALL;
}
bool is_m2m_p2p_cmd(uint8_t cmd) {
  return cmd >= M2M_P2P_CMD_BASE && cmd < M2M_WIFI_MAX_P2P_ALL;
}
bool is_m2m_server_cmd(uint8_t cmd) {
  return cmd >= M2M_SERVER_CMD_BASE && cmd < M2M_WIFI_MAX_SERVER_ALL;
}

const char *m2_config_cmd_to_string(tenuM2mConfigCmd cmd) {
  switch (cmd) {
  case M2M_WIFI_REQ_RESTART:
    return "M2M_WIFI_REQ_RESTART";
  case M2M_WIFI_REQ_SET_MAC_ADDRESS:
    return "M2M_WIFI_REQ_SET_MAC_ADDRESS";
  case M2M_WIFI_REQ_CURRENT_RSSI:
    return "M2M_WIFI_REQ_CURRENT_RSSI";
  case M2M_WIFI_RESP_CURRENT_RSSI:
    return "M2M_WIFI_RESP_CURRENT_RSSI";
  case M2M_WIFI_REQ_GET_CONN_INFO:
    return "M2M_WIFI_REQ_GET_CONN_INFO";
  case M2M_WIFI_RESP_CONN_INFO:
    return "M2M_WIFI_RESP_CONN_INFO";
  case M2M_WIFI_REQ_SET_DEVICE_NAME:
    return "M2M_WIFI_REQ_SET_DEVICE_NAME";
  case M2M_WIFI_REQ_START_PROVISION_MODE:
    return "M2M_WIFI_REQ_START_PROVISION_MODE";
  case M2M_WIFI_RESP_PROVISION_INFO:
    return "M2M_WIFI_RESP_PROVISION_INFO";
  case M2M_WIFI_REQ_STOP_PROVISION_MODE:
    return "M2M_WIFI_REQ_STOP_PROVISION_MODE";
  case M2M_WIFI_REQ_SET_SYS_TIME:
    return "M2M_WIFI_REQ_SET_SYS_TIME";
  case M2M_WIFI_REQ_ENABLE_SNTP_CLIENT:
    return "M2M_WIFI_REQ_ENABLE_SNTP_CLIENT";
  case M2M_WIFI_REQ_DISABLE_SNTP_CLIENT:
    return "M2M_WIFI_REQ_DISABLE_SNTP_CLIENT";
  case M2M_WIFI_RESP_MEMORY_RECOVER:
    return "M2M_WIFI_RESP_MEMORY_RECOVER";
  case M2M_WIFI_REQ_CUST_INFO_ELEMENT:
    return "M2M_WIFI_REQ_CUST_INFO_ELEMENT";
  case M2M_WIFI_REQ_SCAN:
    return "M2M_WIFI_REQ_SCAN";
  case M2M_WIFI_RESP_SCAN_DONE:
    return "M2M_WIFI_RESP_SCAN_DONE";
  case M2M_WIFI_REQ_SCAN_RESULT:
    return "M2M_WIFI_REQ_SCAN_RESULT";
  case M2M_WIFI_RESP_SCAN_RESULT:
    return "M2M_WIFI_RESP_SCAN_RESULT";
  case M2M_WIFI_REQ_SET_SCAN_OPTION:
    return "M2M_WIFI_REQ_SET_SCAN_OPTION";
  case M2M_WIFI_REQ_SET_SCAN_REGION:
    return "M2M_WIFI_REQ_SET_SCAN_REGION";
  case M2M_WIFI_REQ_SET_POWER_PROFILE:
    return "M2M_WIFI_REQ_SET_POWER_PROFILE";
  case M2M_WIFI_REQ_SET_TX_POWER:
    return "M2M_WIFI_REQ_SET_TX_POWER";
  case M2M_WIFI_REQ_SET_BATTERY_VOLTAGE:
    return "M2M_WIFI_REQ_SET_BATTERY_VOLTAGE";
  case M2M_WIFI_REQ_SET_ENABLE_LOGS:
    return "M2M_WIFI_REQ_SET_ENABLE_LOGS";
  case M2M_WIFI_REQ_GET_SYS_TIME:
    return "M2M_WIFI_REQ_GET_SYS_TIME";
  case M2M_WIFI_RESP_GET_SYS_TIME:
  ...
  case M2M_WIFI_REQ_SET_MAC_MCAST:
    return "M2M_WIFI_REQ_SET_MAC_MCAST";
  case M2M_WIFI_REQ_GET_PRNG:
    return "M2M_WIFI_REQ_GET_PRNG";
  case M2M_WIFI_RESP_GET_PRNG:
    return "M2M_WIFI_RESP_GET_PRNG";
  case M2M_WIFI_REQ_SCAN_SSID_LIST:
    return "M2M_WIFI_REQ_SCAN_SSID_LIST";
  case M2M_WIFI_REQ_SET_GAINS:
    return "M2M_WIFI_REQ_SET_GAINS";
  case M2M_WIFI_REQ_PASSIVE_SCAN:
    return "M2M_WIFI_REQ_PASSIVE_SCAN";
  default:
    return "M2M_UNKNOWN";
  }
}
