...
void LCD::init() {
  ...
  // PA8 = reset
  // PB2 = CS
  gpio_mode_setup(LCD_PORT_RESET, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE,
                  LCD_PIN_RESET);
  gpio_mode_setup(LCD_PORT_CMD_DATA, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE,
                  LCD_PIN_CMD_DATA);
  reset_enable();
  ...
  // Unit sense
  gpio_mode_setup(LCD_PORT_UNITSEL, GPIO_MODE_INPUT, GPIO_PUPD_PULLDOWN,
                  LCD_PIN_UNITSEL);
...
void LCD::spi_init() {
  ...
  spi_init_master(LCD_SPI, SPI_CR1_BAUDRATE_FPCLK_DIV_8,
...
void LCD::clear() { n_log("LCD::clear unimplemented\n"); }
...
void LCD::draw_glyph(const uint8_t *glyph, uint8_t x, uint8_t y) {
  ...
  // Pixels are actually addressed assuming 0,0 is top left when display is
  // vertical
  // To convert;
...
void LCD::draw_icon(const uint8_t *bmp, uint8_t w, uint8_t h, uint8_t x,
                    uint8_t y) {
  n_log("LCD::draw_icon unimplemented\n");
}

void LCD::render() { n_log("LCD::render unimplemented\n"); }
...
void LCD::powerOn() {
  ...
  */
  ...
  write(0xc8); // Common output mode select: reverse direction (last 3 bits are
               // ignored)
  ...
  set_contrast(0x10); // Set contrast, value experimentally determined, can set
                      // to 6-bit value, 0 to 63
...
void LCD::update() {
  ...
  // RH + Lux
  if (Sensors.has_rh()) {
    sprintf(buf, "%2.0f%% RH %2.0f%% LUX", Sensors.get_rh(),
            Sensors.get_brightness());
  } else {
...
void LCD::cs_select() { gpio_clear(LCD_PORT_CS, LCD_PIN_CS); }
void LCD::cs_deselect() { gpio_set(LCD_PORT_CS, LCD_PIN_CS); }

void LCD::reset_enable() { gpio_clear(LCD_PORT_RESET, LCD_PIN_RESET); }
void LCD::reset_disable() { gpio_set(LCD_PORT_RESET, LCD_PIN_RESET); }

void LCD::mode_cmd() { gpio_clear(LCD_PORT_CMD_DATA, LCD_PIN_CMD_DATA); }

void LCD::mode_data() { gpio_set(LCD_PORT_CMD_DATA, LCD_PIN_CMD_DATA); }
